...
CREATE TRIGGER `delete_locationType` AFTER DELETE ON `location_type` FOR EACH ROW BEGIN
            INSERT INTO entity_history (entity_name, entity_id, action, old_values, date_action, user)
            VALUES ('Type de lieux', OLD.id, 'delete', JSON_OBJECT('type', OLD.type, 'symbol', OLD.symbol), NOW(), @current_user_name);
        END
$$
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `insert_locationType` AFTER INSERT ON `location_type` FOR EACH ROW BEGIN
        INSERT INTO entity_history (entity_name, entity_id, action, new_values, date_action, user)
        VALUES ('Type de lieux', NEW.id, 'insert', JSON_OBJECT('type', NEW.type, 'symbol', NEW.symbol), NEW.date_modification, NEW.user_modification);
        END
$$
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `update_locationType` AFTER UPDATE ON `location_type` FOR EACH ROW BEGIN
            INSERT INTO entity_history (entity_name, entity_id, action, old_values, new_values, date_action, user)
            VALUES ('Type de lieux', NEW.id, 'update', JSON_OBJECT('type', OLD.type, 'symbol', OLD.symbol), JSON_OBJECT('type', NEW.type, 'symbol', NEW.symbol), NEW.date_modification, NEW.user_modification);
        END
$$
DELIMITER ;
#------------------------------------------------------------
...
#------------------------------------------------------------
DELIMITER $$
CREATE TRIGGER `delete_partners` AFTER DELETE ON `partners` FOR EACH ROW BEGIN
...
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `insert_partners` AFTER INSERT ON `partners` FOR EACH ROW BEGIN
...
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `update_partners` AFTER UPDATE ON `partners` FOR EACH ROW BEGIN
                DECLARE old_partner_type_name VARCHAR(255);
                DECLARE new_partner_type_name VARCHAR(255);
                SELECT type INTO old_partner_type_name FROM partner_type WHERE id = OLD.type_id;
                SELECT type INTO new_partner_type_name FROM partner_type WHERE id = NEW.type_id;
